...
import {
  Cloud, Upload, Search, Grid, List, MoreVertical, Download,
  Share2, Trash2, FileText, ImageIcon, Video, Music, Archive,
  Users, Plus, LogOut, Settings, History, FolderOpen,
} from "lucide-react"
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <Button onClick={() => setShowCreateWorkspace(true)} className="bg-primary text-primary-foreground hover:bg-primary/90">
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={openFilePicker} className="bg-primary text-primary-foreground hover:bg-primary/90">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <Card className="p-6 bg-card border-border hover:border-primary/50 transition-all cursor-pointer">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-primary/10 rounded-lg"><FolderOpen className="h-6 w-6 text-primary" /></div>
              <div><h3 className="font-semibold text-foreground">My Files</h3><p className="text-sm text-muted-foreground">156 files</p></div>
            </div>
          </Card>
          <Card className="p-6 bg-card border-border hover:border-primary/50 transition-all cursor-pointer">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-primary/10 rounded-lg"><Share2 className="h-6 w-6 text-primary" /></div>
              <div><h3 className="font-semibold text-foreground">Shared with me</h3><p className="text-sm text-muted-foreground">23 files</p></div>
            </div>
          </Card>
          <Link href="/workspace/join">
            <Card className="p-6 bg-card border-border hover:border-primary/50 transition-all cursor-pointer">
              <div className="flex items-center gap-4">
                ...
                <div><h3 className="font-semibold text-foreground">Workspaces</h3><p className="text-sm text-muted-foreground">5 active</p></div>
              </div>
            </Card>
          </Link>
        </div>
            ...
            {files.map((file) => (
              <Card key={file.id} className="p-6 bg-card border-border hover:border-primary/50 transition-all group">
                <div className="flex items-start justify-between mb-4">
                  <div className="p-3 bg-primary/10 rounded-lg"><file.icon className="h-8 w-8 text-primary" /></div>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                    ...
                    <DropdownMenuContent align="end" className="bg-card border-border">
                      <DropdownMenuItem className="text-foreground"><Download className="h-4 w-4 mr-2" />Download</DropdownMenuItem>
                      <DropdownMenuItem className="text-foreground"><Share2 className="h-4 w-4 mr-2" />Share</DropdownMenuItem>
                      <DropdownMenuItem className="text-foreground"><History className="h-4 w-4 mr-2" />Version History</DropdownMenuItem>
                ...
                <div key={file.id} className="p-4 hover:bg-secondary/50 transition-colors group">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 flex-1">
                      <div className="p-2 bg-primary/10 rounded-lg"><file.icon className="h-5 w-5 text-primary" /></div>
